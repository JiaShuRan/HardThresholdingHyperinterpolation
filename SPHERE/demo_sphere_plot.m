...
FS = 1 + (scale/(Fmax-Fmin))*(abs(pXYZR(:,5)-gXYZR)-Fmin);
fg = trisurf(tri,XR.*FS, YR.*FS, ZR.*FS, abs(pXYZR(:,5)-gXYZR),'facecolor','interp');
set(fg,'EdgeColor', 'none');
title('\textbf{Absolute error}','interpreter','latex','fontsize',fontsize_baselinet,'position',[0,0,1.45])
colormap(jet(255));
view(-25,25), axis vis3d, axis equal tight, colorbar('south'),caxis([min(abs(pXYZR(:,3)-gXYZR)),max(abs(pXYZR(:,3)-gXYZR))]),
axis off
...
FS = 1 + (scale/(Fmax-Fmin))*(abs(pXYZR(:,6)-gXYZR)-Fmin);
fg = trisurf(tri,XR.*FS, YR.*FS, ZR.*FS, abs(pXYZR(:,6)-gXYZR),'facecolor','interp');
set(fg,'EdgeColor', 'none');
title('\textbf{Absolute error}','interpreter','latex','fontsize',fontsize_baselinet,'position',[0,0,1.45]),
colormap(jet(255));
view(25,25), axis vis3d, axis equal tight, colorbar('south'),caxis([min(abs(pXYZR(:,3)-gXYZR)),max(abs(pXYZR(:,3)-gXYZR))]),
