...
function  [AEinfMV,AE2MV,beta0MV,lambdaV,XYZW,XYZWR,JzMV,HzMV,Wg_norm2]=...
    demo_sphere(lambda_index,a,sigma,XYZW,XYZWR)
...
for k=1:length(NV)
    ...
    if nargin < 4, XYZW=cub_sphere(N); end
    if isempty(XYZW), XYZW=cub_sphere(N); end
    X=XYZW(:,1); Y=XYZW(:,2); Z=XYZW(:,3); W=XYZW(:,4);

    % Define quadrature rule for L2 error at degree NR.
    if nargin < 5, XYZWR=cub_sphere(NR); end
    if isempty(XYZWR), XYZWR=cub_sphere(NR); end
    ...
    for j=1:ntests
        ...
        for ktest=1:6
            ...
            [VR,degs]=vandermonde_sphharm(L,[XR YR,ZR]);
            pXYZR=VR*coeff;
